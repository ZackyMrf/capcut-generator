...
export class CapCut {
  ...
  generateCommerceSign(url, pf = '7', appvr = '12.4.0', tdid = '', timestamp = null) {
    const ts = timestamp || Math.floor(Date.now() / 1000);
    const raw = `9e2c|${url.slice(-7)}|${pf}|${appvr}|${ts}|${tdid}|11ac`;
    const sign = crypto.createHash('md5').update(raw).digest('hex').toLowerCase();
    return { sign, deviceTime: ts };
  }

  getCommerceHeaders(url, cookieString = null, extraHeaders = {}) {
    const effectiveCookie = cookieString || this.cookie;
    const pf = '7';
    const appvr = '12.4.0';
    const tdid = '';
    const { sign, deviceTime } = this.generateCommerceSign(url, pf, appvr, tdid);
    const headers = {
      'Content-Type': 'application/json',
      'Accept': 'application/json, text/plain, */*',
      'User-Agent': this.userAgent,
      'Referer': `${this.apiBase}/commerce/subscription`,
      'Origin': this.apiBase,
      'appid': this.aid,
      'loc': 'sg',
      'lan': 'en',
  ...
  async getCommercePriceList(cookieString = null, options = {}) {
    const effectiveCookie = cookieString || this.cookie;
    const path = '/commerce/v1/subscription/cc_price_list';
    const url = `${this.commerceApiBase}${path}`;
    const headers = this.getCommerceHeaders(path, effectiveCookie);
    const payload = {
      aid: Number(this.aid),
      gp_unavailable: true,
      version: 'v3',
    ...
    if (!res.ok) {
      throw new Error(`Commerce price list error: HTTP ${res.status}`);
    }

    const data = await res.json();
    if (data.ret !== '0') {
      throw new Error(`Commerce price list error: ${data.errmsg || data.ret}`);
    }
  ...
  async getFullAccountProfile(cookieString = null, progressCb = () => {}) {
    ...
    return {
      ...
      referral,
      checkout: this.buildCheckoutUrls({
        userId,
        workspaceId,
  ...
  async getTrial7d(cookieString = null, options = {}, progressCb = () => {}) {
    ...
    return {
      ...
      profile,
      checkout: this.buildCheckoutUrls({
        userId: profile?.userId || options.userId,
        workspaceId: profile?.workspace?.workspaceId,
        code: targetCode || options.referralInput,
        language: options.language || 'en'
      }),
      actions
    };
  ...
  getGoogleAuthUrl(options = {}) {
    const clientId = options.clientId || '87571557143-buc0jlq1ncffscght599nhlgl1u977cr.apps.googleusercontent.com';
    const redirectUri = options.redirectUri || `${this.apiBase}/third-party-callback`;
    const responseType = options.responseType || 'token id_token';
    const scope = options.scope || 'openid https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email';
    const nonce = crypto.randomBytes(16).toString('hex');
    const u = new URL('https://accounts.google.com/o/oauth2/v2/auth');
    u.searchParams.append('client_id', clientId);
    u.searchParams.append('redirect_uri', redirectUri);
    u.searchParams.append('response_type', responseType);
    u.searchParams.append('scope', scope);
    u.searchParams.append('prompt', 'select_account');
    ...
    return {
      ...
      responseType,
      scope,
      instructions: 'Open authUrl in browser, sign in with your Google or GSuite account, and copy the id_token, credential or code.'
    };
  }

  buildCheckoutUrls(options = {}) {
    const lng = options.language || options.lng || 'en';
    const pageFrom = options.pageFrom || 'web';
    const aid = options.aid || this.aid;
    const code = options.code || options.preExchangeCode || options.promoCode || '';
    const userId = options.userId || '';
    const workspaceId = options.workspaceId || '';
    const darkMode = Boolean(options.darkMode);

    const trialUrl = new URL(`${this.apiBase}/capcut_pc_web/fission_receive`);
    trialUrl.searchParams.append('enter_from', 'share');
    trialUrl.searchParams.append('lng', lng);
    if (userId) trialUrl.searchParams.append('user_id', userId);
    if (code) trialUrl.searchParams.append('code', code);

    const subUrl = new URL(`${this.apiBase}/commerce/subscription`);
    subUrl.searchParams.append('lng', lng);
    subUrl.searchParams.append('page_from', pageFrom);
    subUrl.searchParams.append('aid', String(aid));
    subUrl.searchParams.append('locate_tab', 'pro');
    if (code) subUrl.searchParams.append('pre_exchange_code', code);
    if (workspaceId) subUrl.searchParams.append('workspace_id', workspaceId);
    if (darkMode) subUrl.searchParams.append('dark_mode', 'true');

    const activateUrl = new URL(`${this.apiBase}/commerce/vip-activate`);
    activateUrl.searchParams.append('page_from', pageFrom);
    activateUrl.searchParams.append('lng', lng);
    if (code) activateUrl.searchParams.append('activation_code', code);

    const teamUrl = new URL(`${this.apiBase}/commerce/subscription`);
    teamUrl.searchParams.append('lng', lng);
    teamUrl.searchParams.append('page_from', 'workspace');
    teamUrl.searchParams.append('aid', String(aid));
    if (workspaceId) teamUrl.searchParams.append('workspace_id', workspaceId);

    const priceListUrl = `${this.commerceApiBase}/commerce/v1/subscription/cc_price_list?aid=${aid}`;

    return {
      freeTrial7dUrl: trialUrl.toString(),
      trial7dUrl: trialUrl.toString(),
      webSubscriptionUrl: subUrl.toString(),
      pcFissionTrialUrl: trialUrl.toString(),
      vipActivateUrl: activateUrl.toString(),
      teamCheckoutUrl: teamUrl.toString(),
      priceListUrl
    };
  }

  async registerOrLoginWithGoogle(googleInput = {}, options = {}, progressCb = () => {}) {
    const input = typeof googleInput === 'string' ? { token: googleInput } : { ...googleInput };
    const mergedOptions = { ...options, ...input };

    const token = mergedOptions.token || mergedOptions.idToken || mergedOptions.credential || mergedOptions.googleToken || '';
    const code = mergedOptions.code || mergedOptions.authCode || mergedOptions.googleCode || '';
    const accessToken = mergedOptions.accessToken || '';
    const platformAppId = mergedOptions.platformAppId || '1773';

    if (!token && !code && !accessToken) {
      throw new Error('Google authentication requires a token (ID token / credential / secret) or OAuth authorization code');
    }

    let inviteCode = mergedOptions.inviteCode || null;
    let inviterUserId = mergedOptions.inviterUserId || null;

    if (mergedOptions.referralInput) {
      const refStr = String(mergedOptions.referralInput || '').replace(/[\r\n\t\s]+/g, '').trim();
    ...
    if (inviteCode || inviterUserId) {
      const bizObj = {};
      if (inviteCode) bizObj.invite_code = inviteCode;
      if (inviterUserId) bizObj.inviter_uid = inviterUserId;
      bizObj.enter_from = 'share';
      bizParam = encodeURIComponent(JSON.stringify(bizObj));
    }

    progressCb('Authenticating with CapCut Passport via Google / GSuite...');
    const url = new URL(`${this.apiBase}/passport/web/auth/login/`);
    url.searchParams.append('aid', this.aid);
    url.searchParams.append('account_sdk_source', 'web');
    url.searchParams.append('language', 'en');
    url.searchParams.append('check_region', '1');
    ...
    formData.append('platform_app_id', String(platformAppId));
    formData.append('biz_param', bizParam);

    if (token) {
      formData.append('access_token_secret', token);
    ...
    const json = await res.json();
    if (json.message !== 'success' || !json.data) {
      const errMsg = json.data?.description || json.data?.dialog_tips || json.message || JSON.stringify(json);
      throw new Error(`Google / GSuite authentication failed: ${errMsg}`);
    }
    ...
    const userId = userData.user_id_str || String(userData.user_id || '');
    progressCb(`Google authentication successful: User ID ${userId}`);

    let claimResult = null;
    if (mergedOptions.referralInput || inviteCode || inviterUserId) {
      const refTarget = mergedOptions.referralInput || inviteCode || inviterUserId;
      progressCb(`Processing referral claim for: ${refTarget}...`);
      try {
        claimResult = await this.claimReferral(refTarget, cookieString, progressCb);
      } catch (e) {
    ...
    let joinedTeam = null;
    if (mergedOptions.team || mergedOptions.teamLink || mergedOptions.spaceLink) {
      const teamTarget = mergedOptions.team || mergedOptions.teamLink || mergedOptions.spaceLink;
      progressCb(`Processing team workspace invitation for: ${teamTarget}...`);
      try {
        joinedTeam = await this.joinWorkspace(teamTarget, cookieString, progressCb);
      } catch (e) {
        progressCb(`Team join note: ${e.message}`);
      }
    }

    progressCb('Fetching complete account profile and pro details...');
    let fullProfile = null;
    try {
      fullProfile = await this.getFullAccountProfile(cookieString, progressCb);
    } catch {
      fullProfile = {
        userId,
        secUserId: userData.sec_user_id || '',
        screenName: userData.screen_name || userData.name || '',
        email: userData.email || '',
        role: 'owner',
        isPro: false,
        pro: { isPro: false, level: 'free', startTime: null, expireTime: null },
        isNewUser: Boolean(userData.new_user),
        region: 'SG',
        referral: {
          referralId: userId,
          userId,
          secUserId: userData.sec_user_id || '',
          referralLink: `${this.apiBase}/capcut_pc_web/fission_receive?enter_from=share&user_id=${userId}`
        }
      };
    }

    let trialResult = null;
    const shouldGetTrial = mergedOptions.getTrial !== false;
    if (shouldGetTrial) {
      progressCb('Claiming 7-day Pro free trial for Google account...');
      try {
        trialResult = await this.getTrial7d(cookieString, {
          userId: fullProfile?.userId || userId,
          inviterUid: inviterUserId,
          inviteCode,
          referralInput: mergedOptions.referralInput,
          region: mergedOptions.region || 'SG'
        }, progressCb);
    ...
    const checkout = this.buildCheckoutUrls({
      userId: fullProfile?.userId || userId,
      workspaceId: fullProfile?.workspace?.workspaceId,
      code: inviteCode || mergedOptions.referralInput,
      language: mergedOptions.language || 'en'
    });

    progressCb('Google account setup completed successfully.');
    return {
      authProvider: 'google',
      email: fullProfile.email || userData.email || '',
      userId: fullProfile.userId || userId,
      screenName: fullProfile.screenName || userData.screen_name || '',
      avatarUrl: userData.avatar_url || '',
      secUserId: fullProfile.secUserId || userData.sec_user_id || '',
  ...
  async registerDisposableAccount(options = {}, progressCb = () => {}) {
    ...
    let code = options.code || options.otp || null;
    if (!code) {
      if (options.email && typeof options.onOtpRequest === 'function') {
        code = await options.onOtpRequest(email);
      } else {
        progressCb('Waiting for verification code in inbox...');
        const timeout = options.timeoutMs || 60000;
        code = await waitForVerificationCode(email, timeout);
      }
    }
    ...
    const checkout = this.buildCheckoutUrls({
      ...
      workspaceId: fullProfile.workspace?.workspaceId,
      code: inviteCode || options.referralInput,
      language: options.language || 'en'
    });
