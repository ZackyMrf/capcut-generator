...
export class CapCut {
  ...
  async claimReferral(referralInput, cookieString = null, progressCb = () => {}) {
    ...
    const input = String(referralInput).trim();
    ...
    if (isUrl) {
      progressCb(`Visiting referral link ${input}...`);
      try {
      ...
      const fissionUrl = `${this.apiBase}/capcut_pc_web/fission_receive?enter_from=share&code=${encodeURIComponent(inviteCode)}`;
      progressCb(`Calling fission receiver with code ${inviteCode}...`);
      try {
        const res = await fetch(fissionUrl, {
          headers: this.buildHeaders({ 'Cookie': effectiveCookie }),
          redirect: 'follow'
        });
        results.actions.push({
          action: 'fission_receive',
          status: res.status,
          success: res.ok
        });
      } catch (e) {
  ...
  async registerDisposableAccount(options = {}, progressCb = () => {}) {
    ...
    if (options.referralInput) {
      const refStr = String(options.referralInput).trim();
